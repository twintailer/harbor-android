// Modules that work on every platform (network, filesystem, pure Rust).
mod airplay;
mod cast;
mod cast_hls;
mod cast_server;
...
// Desktop-only modules: libmpv playback, auxiliary windows, tray, Discord IPC,
// external binaries (ffmpeg/mpv/yt-dlp helpers that manage desktop windows).
#[cfg(desktop)]
mod anime4k;
#[cfg(desktop)]
mod browser;
...
async fn deeplink_set_stremio(app: tauri::AppHandle, enabled: bool) -> Result<(), String> {
    ...
    #[cfg(mobile)]
    {
        // URL schemes are declared statically in Info.plist / AndroidManifest.
        let _ = (&app, enabled);
    }
    Ok(())
}
